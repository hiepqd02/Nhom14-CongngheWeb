...
const create = async (title,
    ...
    try {
        ...
        const validate = await utils.validateCardOwners(null, list, board, user, true);
...
const setChecklistItemCompleted = async (
    ...
    try {
        ...
        board.activity.unshift({
            ...
            name: user.name,
            action: completed
                ? `completed '${itemText}' on ${card.title}`
                : `marked as uncompleted to '${itemText}' on ${card.title}`,
            color: user.color,
        });
        board.save();
